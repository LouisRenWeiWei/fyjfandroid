...
//            .getExternalStorageDirectory().getPath();
     ...
     * @param updateInfo
     */
    private void showNetDialog(final AppVersion updateInfo) {
     ...
     * @param updateInfo
     */
    private void showUpdateUI(final AppVersion updateInfo) {
        final int type = NetworkUtils.getNetworkType(mContext);
     ...
     * 检查更新任务
     */
    private class AsyncCheck extends AsyncTask<String, Integer, AppVersion> {
        @Override
        ...
        protected AppVersion doInBackground(String... params) {
            ...
            String url = params[0];
            try {
                InputStream is = new HttpRequest(url).get();
                ...
                updateInfo = JSONUtil.toBean(result.getJSONObject("data"),AppVersion.class);
            } catch (Exception e) {
                e.printStackTrace();
                return null;
            }
     ...
     * 异步下载app任务
     */
    private class AsyncDownLoad extends AsyncTask<AppVersion, Integer, Boolean> {
        @Override
        protected void onPreExecute() {
